...
public abstract class KeyVaultInterface {
     ...
     *         For example, KeyVaultWindowsDpapi returns "dpapi" and KeyVaultMac returns "mac".
     */
    public abstract String getId();

    /**
     * Get the human readable version of the key vault identifier (ISKeyVaultInterface::getId()).
     * @return Returns an identification string that is unique amongst all key vault implementations.
     *         For example, ISKeyVaultWindowsDpapi returns "Windows DPAPI Key Vault" and ISKeyVaultMac returns
     *         "Mac OS X Key Vault".
     */
    ...
    /**
     * Add or update a key into the key vault.  (See SetKey(..) for details.)
     *
     *
     ...
     * @return Returns a code from ISKeyVaultError.h. See details section for all error code
     *         possibilities of this function.
     ...
     *
     *   If the key is found, but its 'issued' time (KeyVaultKey::getIssuedServerTimeUtcSeconds())
     ...
     *
     * Searches for a key identified by sKeyId.  On success, keyOut will be populated
     * with the found key and ISKEYVAULT_OK will be returned. If the key is not found,
     * then null will be returned.
     * @param sKeyId The data protection key ID (also known as the key tag).
     * @return Output key object that is populated with the retrieved key or null.
     * @throws IonicException on errors listed in the description
     */
    public abstract KeyVaultKey getKey(String sKeyId) throws IonicException;

    /**
     * Get one or more keys from the key vault.
     *
     * Searches for all the key IDs specified in setKeyIds. vecKeysOut will be
     * populated with all the keys which were found and ISKEYVAULT_OK will be returned.
     * There is no failure condition for this function; it always returns ISKEYVAULT_OK even
     * if zero keys are found.
     * @param setKeyIds The set of data protection key IDs (also known as the key tag).
     * @return Output vector of key objects that is populated with the retrieved keys.
     */
     ...
     *
     * Populates setKeyIdsOut with all data protection key IDs that are
     * contained in the key vault.
     ...
     *
     * Populates vecKeysOut with all data protection keys that are
     * contained in the key vault.
     ...
     * @param sKeyId The data protection key ID to look for.
     * @return  Returns true if a key with the specified ID exists.  Otherwise, returns false.
     */
    public abstract boolean hasKey(String sKeyId);
     ...
     * KeyVaultInterface::removeKey(String) with the key ID of the input
     ...
     *          not yet been put into permanent storage via ISKeyVaultInterface::sync(). For example, if
     *          a key is added, update, or removed, then a call to ISKeyVaultInterface::sync() is needed
     ...
     *
     *          The key vaults provided by Ionic (e.g. ISKeyVaultWindowsDpapi, ISKeyVaultMac) all
