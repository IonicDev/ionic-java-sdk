...
/**
 * Test ability to perform file crypto operations using {@link GenericFileCipher}.
 */
public class GenericFileCipherTest {
    ...
    /**
     * Test {@link GenericFileCipher} helper APIs.
     *
     * @throws IonicException on failure to initialize Ionic library
     */
    @Test
    public final void testGenericCipher_HelperAPIs_Success() throws IonicException {
        ...
        Assert.assertEquals(FileCipher.Generic.V13.LABEL, fileCipher.getDefaultVersion());
        Assert.assertEquals(CipherFamily.FAMILY_GENERIC, fileCipher.getCipherFamily());
        Assert.assertEquals(FileCipher.Generic.FAMILY, fileCipher.getFamilyString());
        Assert.assertTrue(fileCipher.isVersionSupported(FileCipher.Generic.V12.LABEL));
        Assert.assertTrue(fileCipher.isVersionSupported(FileCipher.Generic.V13.LABEL));
        Assert.assertFalse(fileCipher.isVersionSupported(FileCipher.Csv.V10.LABEL));
    }

    /**
     * Test {@link GenericFileCipher} byte[] API operations.
     *
     ...
     */
    @Test
    public final void testGenericCipherBytes_EncryptDecrypt_CodecSymmetry() throws IonicException, IOException {
        ...
        for (int count = countMin; (count <= countMax); count *= step) {
            ...
            Assert.assertEquals(CipherFamily.FAMILY_GENERIC, fileInfoPlain.getCipherFamily());
            Assert.assertEquals(FileCipher.Generic.V13.LABEL, fileInfoPlain.getCipherVersion());
            ...
            for (String version : versions) {
                ...
                Assert.assertEquals(CipherFamily.FAMILY_GENERIC, fileInfoCipher.getCipherFamily());
    ...
    /**
     * Test {@link GenericFileCipher} path API operations.
     *
     * @throws IonicException on failure to initialize Ionic library, on cryptography operation failures
     * @throws IOException    on data generation failure, filesystem I/O failure
     */
    @Test
    public final void testGenericCipherFile_EncryptDecrypt_CodecSymmetry() throws IonicException, IOException {
        logger.info(String.format("PROCESSOR ARCHITECTURE = %s", System.getProperty(VM.Sys.OS_ARCH)));
        final KeyServices keyServices = IonicTestEnvironment.getInstance().getKeyServices();
        final FileCipherAbstract fileCipher = new GenericFileCipher(keyServices);
    ...
    /**
     * Test {@link GenericFileCipher} constructor that accepts a {@link FileCryptoCoverPageServicesInterface}.
     *
     * @throws IonicException on failure to initialize Ionic library, on cryptography operation failures
     */
    @Test
    public final void testGenericCipherFile_CtorCoverPage_ExpectedBehavior() throws IonicException {
        final KeyServices keyServices = IonicTestEnvironment.getInstance().getKeyServices();
        final FileCryptoCoverPageServicesInterface coverPageServices = new FileCryptoCoverPageServicesDefault();
        final FileCipherAbstract fileCipher = new GenericFileCipher(keyServices, coverPageServices);
