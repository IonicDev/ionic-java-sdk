...
/**
 * Test ability to perform file crypto operations using {@link PdfFileCipher}.
 */
public class PdfFileCipherTest {
    ...
    /**
     * Test {@link PdfFileCipher} helper APIs.
     *
     * @throws IonicException on failure to initialize Ionic library
     */
    @Test
    public final void testPdfCipher_HelperAPIs_Success() throws IonicException {
        ...
        Assert.assertEquals(FileCipher.Pdf.V10.LABEL, fileCipher.getDefaultVersion());
        Assert.assertEquals(CipherFamily.FAMILY_PDF, fileCipher.getCipherFamily());
        Assert.assertEquals(FileCipher.Pdf.FAMILY, fileCipher.getFamilyString());
        Assert.assertTrue(fileCipher.isVersionSupported(FileCipher.Pdf.V10.LABEL));
        Assert.assertFalse(fileCipher.isVersionSupported(FileCipher.Generic.V12.LABEL));
    }

    /**
     * Test {@link PdfFileCipher} byte[] API operations.
     *
     ...
     */
    @Test
    public final void testPdfCipherBytes_EncryptDecrypt_CodecSymmetry() throws IonicException, IOException {
        ...
        for (int count = countMin; (count <= countMax); count *= step) {
            ...
            for (String version : versions) {
                ...
                Assert.assertEquals(CipherFamily.FAMILY_PDF, fileInfoCipher.getCipherFamily());
    ...
    /**
     * Test {@link PdfFileCipher} path API operations.
     *
     * @throws IonicException on failure to initialize Ionic library, on cryptography operation failures
     * @throws IOException    on data generation failure, filesystem I/O failure
     */
    @Test
    public final void testPdfCipherFile_EncryptDecrypt_CodecSymmetry() throws IonicException, IOException {
        logger.info(String.format("PROCESSOR ARCHITECTURE = %s", System.getProperty(VM.Sys.OS_ARCH)));
        final KeyServices keyServices = IonicTestEnvironment.getInstance().getKeyServices();
        final FileCipherAbstract fileCipher = new PdfFileCipher(keyServices);
    ...
    /**
     * Test {@link PdfFileCipher} constructor that accepts a {@link FileCryptoCoverPageServicesInterface}.
     *
     * @throws IonicException on failure to initialize Ionic library, on cryptography operation failures
     */
    @Test
    public final void testPdfCipherFile_CtorCoverPage_ExpectedBehavior() throws IonicException {
        final KeyServices keyServices = IonicTestEnvironment.getInstance().getKeyServices();
        final FileCryptoCoverPageServicesInterface coverPageServices = new FileCryptoCoverPageServicesDefault();
        final FileCipherAbstract fileCipher = new PdfFileCipher(keyServices, coverPageServices);
    ...
    /**
     * Test coverage for a document containing an xref stream.
     *
     * @throws IOException    on channel seek / read failure
     * @throws IonicException on data expectation failure
     */
    @Test
    public final void testPdfCipherFile_XrefStream_ExpectedBehavior() throws IonicException, IOException {
        final URL url = Resource.resolve("com/ionic/sdk/agent/cipher/file/pdf/Office Layout (US units)_PDFE.pdf");
        Assert.assertNotNull(url);
        final byte[] plainTextPdf = DeviceUtils.read(url);
        final SeekableByteChannel channel = new SeekableByteBufferChannel(plainTextPdf);
        final IonicPdfDocument pdfDocument = new IonicPdfDocument(channel, plainTextPdf.length);
        pdfDocument.initialize();
        logger.info(pdfDocument.getXrefTable().getTrailerDictionary().toString());
    }
